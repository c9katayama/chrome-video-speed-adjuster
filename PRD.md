...
| rewindTime | Yキーで巻き戻す秒数 | 10 | 1～60秒 |
| advanceTime | Hキーで早送りする秒数 | 10 | 1～60秒 |
| smallAdvanceTime | Gキーで早送りする秒数 | 5 | 1～60秒、ユーザー設定可 |
| smallRewindTime | Tキーで巻き戻す秒数 | 5 | 1～60秒、ユーザー設定可 |
...
| keyBindings | 各機能のキーバインド設定 | - | W, D, R, Y, H, G, T, V など |

### キーバインド詳細
- speedDown: 再生速度を下げる（W）
- speedUp: 再生速度を上げる（D）
- resetSpeed: 再生速度をリセット（R）
- rewind: 指定秒数巻き戻し（Y）
- advance: 指定秒数早送り（H）
- smallAdvance: 小刻み早送り（G）
- smallRewind: 小刻み巻き戻し（T）
